...
## 🧪 Что можно показать преподавателю за 2 минуты

1. `./scripts/setup.sh` — стек поднимается в один запуск.
2. Открыть http://localhost:8080/panel — обзорная панель.
3. **Серверы** → «Добавить в базу» → строка появляется в таблице → «Изменить» → «Удалить».
4. **Инциденты** → «Принять», затем «Закрыть» — статус в строке обновляется.
5. **Правила алертов** → правим `min`/`max`, переключаем «Вкл/Выкл».
6. **Настройки** → меняем `retention_days`, нажимаем «Сохранить».
7. **Отчёты** → «Скачать CSV» — выгружается файл с SLA-витриной.
